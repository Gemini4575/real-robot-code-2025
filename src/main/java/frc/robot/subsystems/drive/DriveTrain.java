...
public class DriveTrain extends SubsystemBase {
  ...
  public void configureAutoBuilder() {
    ...
    AutoBuilder.configure(
        ...
        (speeds, feedforwards) -> driveForPathPlanner(speeds), // Method that will drive the robot given ROBOT RELATIVE
  ...
  public void driveRobotRelative(ChassisSpeeds chassisSpeedsIn) {
    ...
    drive(chassisSpeedsIn.vxMetersPerSecond, chassisSpeedsIn.vyMetersPerSecond, chassisSpeedsIn.omegaRadiansPerSecond,
        false);
  }
  ...
  // var speeds = ChassisSpeeds.discretize(chassisSpeedsIn,
  ...
  // startencoder =
  // Math.abs(java.util.Arrays.stream(encoderDoubles).mapToDouble(Double::doubleValue).average().orElse(0.0));
  // }
  // curencoder =
  // Math.abs(java.util.Arrays.stream(curencoderDoubles).mapToDouble(Double::doubleValue).average().orElse(0.0));
  // target = Math.abs((startencoder + rotationsToInch.calculateRotationsM(meters,
  // 6.75, 6)) -
  // java.util.Arrays.stream(curencoderDoubles).mapToDouble(Double::doubleValue).average().orElse(0.0));
  ...
  public void periodic() {
     ...
     * 
     * // this is probably not the right speed calculation.. need to figure out
     * where to get the speed
     * m_frontLeft.getDriveMotorSim().iterate(xSpeed_cur + ySpeed_cur,
     * m_frontLeft.getDriveMotorSim().getBusVoltage(), 0.02);
     * m_frontRight.getDriveMotorSim().iterate(xSpeed_cur + ySpeed_cur,
     * m_frontRight.getDriveMotorSim().getBusVoltage(), 0.02);
     * 
     * simDrive.setInputs(m_frontLeft.getDriveMotorSim().getAppliedOutput() *
     * m_frontLeft.getDriveMotorSim().getBusVoltage(),
     * m_frontRight.getDriveMotorSim().getAppliedOutput() *
     * m_frontRight.getDriveMotorSim().getBusVoltage());
